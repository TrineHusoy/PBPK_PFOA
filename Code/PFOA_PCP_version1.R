...
# change from characers to numeric
...
PFOAconcPCP_LB_male <- PFOAconcPCP_LB
PFOAconcPCP_MB_male <- PFOAconcPCP_MB
PFOAconcPCP_UB_male <- PFOAconcPCP_UB
...
# Amount of PFOA (total gram used * ng/g conc /1000 = ug)


amount_PFOA_m_LB <- use_amount_m
amount_PFOA_m_LB <- compound_amount(PFOAconcPCP_LB_male, amount_PFOA_m_LB, use_amount_m)/1000 # convert to ug

amount_PFOA_m_MB <- use_amount_m
amount_PFOA_m_MB <- compound_amount(PFOAconcPCP_MB_male, amount_PFOA_m_MB, use_amount_m)/1000

amount_PFOA_m_UB <- use_amount_m
amount_PFOA_m_UB <- compound_amount(PFOAconcPCP_UB_male, amount_PFOA_m_UB, use_amount_m)/1000
...
amount_PFOA_m_LB_eef <- amount_rinse(EEFsMale,amount_PFOA_m_LB_eef, amount_PFOA_m_LB ) # ug
...
total_m_ID_LB[,2:1001] <- total_m_LB[,1:1000] # in ug/day
...
total_m_ID_MB[,2:1001] <- total_m_MB[,1:1000] # in ug/day
...
total_m_ID_UB[,2:1001] <- total_m_UB[,1:1000] # in ug/day
...
Plot_pfoa_m_pcp <- ggplot(data = pfoa_m_pcp, aes(PFOAconc, colour = Exposure)) +
  ...
  scale_colour_hue()+
  theme_minimal()+
  scale_x_continuous(limits=c(0.0001,0.01))+
  xlab("PFOA (ug/day)")+
  ylab("Cumulative probability")+
  annotate("text", x=0.009, y=0.85, label="Males", size=6)
...
amount_PFOA_f_LB <- compound_amount(PFOAconcPCP_LB_female, amount_PFOA_f_LB, use_amount_f)/1000 # convert to ug

amount_PFOA_f_MB <- use_amount_f
amount_PFOA_f_MB <- compound_amount(PFOAconcPCP_MB_female, amount_PFOA_f_MB, use_amount_f)/1000

amount_PFOA_f_UB <- use_amount_f
amount_PFOA_f_UB <- compound_amount(PFOAconcPCP_UB_female, amount_PFOA_f_UB, use_amount_f)/1000
...
###### calculate the exposure in ug/day # This can be the input for the PBPK modelling ########
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

total_f_ID_LB <- as.data.frame(matrix(NA, nrow = 100, ncol = 1001))
total_f_ID_LB[,1] <- ProductUsePCP_female[,1] #get ID 
total_f_ID_LB[,2:1001] <- total_f_LB[,1:1000] # in ug/day
...
total_f_ID_MB[,2:1001] <- total_f_MB[,1:1000] # in ug/day
...
total_f_ID_UB[,2:1001] <- total_f_UB[,1:1000] # in ug/day
...
Plot_pfoa_f_pcp <- ggplot(data = pfoa_f_pcp, aes(PFOAconc, colour = Exposure)) +
  ...
  scale_colour_hue()+
  theme_minimal()+
  scale_x_continuous(limits=c(0.001,0.015))+
  xlab("PFOA (ug/day)")+
  ylab("Cumulative probability")+
  annotate("text", x=0.013, y=0.80, label="Females", size=6)
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
##  Combining plot and data for males and females and LB, MB and UB
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
SumPFOA_LB_PCP <- rbind(Sumtotal_f_ID_LB_kg, Sumtotal_m_ID_LB_kg)
SumPFOA_LB_PCP <- SumPFOA_LB_PCP %>% dplyr::select(-N)
SumPFOA_LB_PCP <- SumPFOA_LB_PCP %>%  rename_with(.fn = function(.x){paste0( .x, "_dermal")},
